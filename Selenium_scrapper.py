...
# import requests
# html_doc = requests.get("http://www.nexiilabs.com/")
...
class Xpath_Util:
    ...
    def __init__(self):
        self.elements = None
        self.guessable_elements = ['input','button','div','a']
                        ...
                        for attr in self.known_attribute_list:
                            if element.has_attr(attr):                                                          
                                ...
                                if len(driver.find_elements_by_xpath(locator))==1:
                                    result_flag = True
                                    print('===============================')                                   
                                    print (locator.encode('utf-8'),element)
                                    print('===============================')
                                    break
    ...
    def guess_xpath(self,tag,attr,element):
        ...
        if type(element[attr]) is list:
            # element[attr] = [i.decode('utf-8') for i in element[attr]]
            x= ''
            for y in element[attr]:
                x= x +str(y)
            element[attr]= x
